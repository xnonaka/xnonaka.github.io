...
Based on the plot of counts at each minute, there is an evident discrepancy in the time data. Upon observing the data, it is clear that for minute multiples of 5 (and especially 10) the count spikes (i.e. 1:30 much higher than 1:29 and 1:31). This can be viewed in the second plot (stop frequency from 8AM to 10AM), where times every 5 minute are blue. It follows that when an individual traffic stop is recorded, the time of day is often rounded to the nearest regular 5th minute. 

```{r}
time_table5 <- time_table %>% 
  mutate(time10 = floor_date(time, unit = "5 minutes")) %>% 
  group_by(time10) %>% 
...
time_table5 %>% 
  ...
  labs(x = "Time", 
       y = "Number of Stops", 
       title = "Number of Stops (Every 5th minute)")
```
...
SELECT time, SUM(arrest_made) AS num_arrests, "San Francisco" AS city 
FROM ca_san_francisco_2020_04_01
GROUP BY time
...
SELECT time, SUM(arrest_made) AS num_arrests, "Oakland" AS city 
...
SELECT time, SUM(arrest_made) AS num_arrests, "San Jose" AS city 
